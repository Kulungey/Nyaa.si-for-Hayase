export default new class Nyaa {
  ...
  async single(query) {
    const { titles, episode, fetch } = query
    if (!titles?.length) return []
    const q = titles[0] + (episode ? ` ${episode}` : '')
    // c=1_2 = Anime - English-translated. Use c=1_0 for all anime categories.
    const url = `${this.base}?page=rss&c=1_2&q=${encodeURIComponent(q)}`
  ...
  parseRSS(text) {
    ...
    while ((match = itemRegex.exec(text)) !== null) {
      ...
      // Note: 'best'/'alt' type is intentionally left unset - per Hayase's
      // extension guidelines, that should only be set for manually verified releases.
      results.push({
        title,
        link: magnet,
        hash,
  ...
  async test() {
    try {
      ...
      if (!res.ok) throw new Error('nyaa.si did not respond OK')
      return true
    } catch (e) {
      throw new Error(`Could not reach nyaa.si: ${e.message}`)
    }
  }
}()
